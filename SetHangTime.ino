...
#ifdef DEBUG   
 Serial.print("tempHangtimer = "); Serial.println(tempHangtimer);
 #endif
   sendStorecmd("HT", tempHangtimer);
   lcd.setCursor(0, 3);
   lcd.print(F("Hang Timer Saved   "));
#ifdef BLE
   char tempCmd[8] = "HT 0000";
   for (int i = 0; i < 4; i ++) {
        if (tempHangtimer[i + 3] == '\0') {
            tempCmd[i + 3] = '0';
        } else {
                tempCmd[i + 3] = tempHangtimer[i];
                }
        tempCmd[i + 4] = '\0';
   }
